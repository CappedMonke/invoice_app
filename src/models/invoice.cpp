...
bool Invoice::save() {
    ...
    std::string query;
    if (id == 0) {
        query = "INSERT INTO invoices (customer_id, date, total_amount) VALUES ('" + std::to_string(customer_id) +
                ", '" + date + "', " + std::to_string(total_amount) + ");";
    } else {
        query = "UPDATE invoices SET customer_id = " + std::to_string(customer_id) + ", date = '" + date +
                "', total_amount = " + std::to_string(total_amount) + " WHERE id = " + std::to_string(id) + ";";
    }
